...
from hospital_management_system.services.departments import DepartmentService
...
class Department(Resource):
    ...
    def post(self):
        ...
        if is_validated:
            return DepartmentService().create_department(args_dict)
        else:
...
class AssignDepartment(Resource):
    ...
    def post(self):
        parser = reqparse.RequestParser(bundle_errors=True)
        parser.add_argument("department_id",type=int,required=True,help="department_id can't be blank")
        parser.add_argument("doctor_id",type=int,required=True,help="doctor_id can't be blank")
        args_dict=parser.parse_args().copy()
        is_validated,err=self.validate(args_dict)
        if is_validated:
            return DepartmentService().assign_department(args_dict)
        else:
            return RestResponse(err=err).to_json(),400
        
class DepartmentServices(Resource):
    @jwt_required()
    @user_exists
    def get(self,department_id):
        return DepartmentService().get_department_service(department_id)